...
--
CREATE DATABASE IF NOT EXISTS `time_table_db` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `time_table_db`;
...
--
ALTER TABLE `dedidicated_course_rooms`
  ADD CONSTRAINT `course` FOREIGN KEY (`course_code`) REFERENCES `courses` (`code`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `room` FOREIGN KEY (`room_id`) REFERENCES `lecture_rooms` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
